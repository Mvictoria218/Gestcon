...
def enviar_usuario():
    ...
    caminho_imagem = None  # Default caso não haja imagem
    if imagem:
    ...
    VALUES (?, ?, ?, ?)
    '''
    cursor.execute(sql, (usuario, senha, nome_usuario, caminho_imagem))

    conexao.commit()
    conexao.close()

    return render_template('cadastro_usuario.html', imagem=nome_imagem)
